...
public class GioHangCTBanTaiQuayController {
    ...
    // ToDo load giỏ hàng chi tiết tại quầy theo idgh của khác
    ...
    public ResponseEntity<?> ThemSPVaoGHCTBanTaiQuay(Principal principal, @RequestParam("idgh")UUID idgh, @RequestParam("idspct") UUID idspct,
        ...
        try {
            if(principal != null){
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Không tìm thấy nhân viên thêm sản phẩm vào ghct bán tại quầy !");
            }
        }catch (Exception e){
    ...
    public ResponseEntity<?> updateSPGioHangCT(Principal principal, @RequestParam("idghct") UUID idghct, @RequestParam("soluong") Integer soluong){
        try {
            if(principal != null){
    ...
    public ResponseEntity<?> deleteSPGioHangCT(Principal principal, @RequestParam("idghct") UUID idghct){
        try {
            ...
            return ResponseEntity.badRequest().body(new MessageGioHangCTRespon ("Lỗi không thế xóa sp trong ghct bán tại quầy !"));
        }
    }
}
